...
<script>
    ...
    */
    function get_dashboard(){
        $.ajax({
            type :'post',
            url: "<?php echo site_url() . '/SqlController/get_cluster_ajax'; ?>",
            data:{
            },
            dataType: 'json',
            success: function(json_data){
            set_dashboard(json_data['json_cluster']);
            }                    
        });
    }
    ...
    <?php 
    	...
    	// echo'<pre>';
        // print_r( $json_data['json_cluster'] );
        // echo'<pre>';
...
<script>
    ...
    function set_dashboard(json_cluster){


    arr_cluster = [];
    arr_score = [];
    // arr_score = [29, 71, 106, 129, 144, 176, 135, 148, 216, 194]




    for(i=0; i < json_cluster.length; i++){
        console.log( json_cluster[i].cluster_name );
        console.log( json_cluster[i].cluster_score );

        arr_cluster.push(json_cluster[i].cluster_name );
        arr_score.push( parseInt(json_cluster[i].cluster_score) );
    }
    ...
    const chart = Highcharts.chart('container', {
        ...
        xAxis: {
            ...
            labels: {
				useHTML: true,
				formatter: function() {
					return '<span><b>'+this.value+'<b></span><br><img src="https://se.buu.ac.th/gami_ossd/assets/dist/img/cluster/cluster'+this.value.substring(this.value.length-1)+'.png" style="width: 50px; vertical-align: middle" />';
				}
			},
			crosshair: true
        },
